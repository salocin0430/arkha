...
export class CreateMission {
  ...
  async execute(request: CreateMissionRequest): Promise<Mission> {
    ...
    if (request.passengers < 1 || request.passengers > 100) {
      throw new Error('Number of passengers must be between 1 and 100');
    }

    if (request.duration < 1 || request.duration > 365) {
      throw new Error('Mission duration must be between 1 and 365 days');
    }
